...
4. **并发推送与历史自动清理**：
   ...
   - 自动运行历史清理工作流，使每个任务严格只保留**最新的 10 次**历史记录，保持仓库极其精简。
5. **Bark 推送支持**：
   - 支持在每次域名解析自动同步完成后，通过 Bark 实时推送同步结果到你的手机上。
   - **动态地区标题**：推送通知标题根据运行的测速地区自动显示为中文名称（如 `【新加坡】优选域名更新成功！`）。
   - **清脆提示音**：默认使用 `glass`（玻璃撞击）提示音，通知提醒更加直观醒目。
...
3. 可以在左侧找到 `一键触发所有测速`，点击 **Run workflow** ➡️ **Run workflow**，即可一键触发并并行运行所有地区（SG、JP、US、DE、NL、US_MAX）的测速与同步工作流。您也可以单独在左侧选择某个地区的工作流单独运行。

> 💡 **工作流运行机制与延迟说明**：
> - **仅通过定时和手动触发**：为了防止 Actions 自动推送代码所引起的无限死循环运行，本项目已去除了 `push` 触发条件。目前工作流仅通过每小时定时自动执行或由您手动触发。
> - **定时任务存在延迟**：由于 GitHub Actions 定时调度（`schedule`）队列的负载均衡机制，任务启动时间通常会有 10 - 50 分钟左右的延迟，各个地区工作流可能不会在同一时间准点启动，这属于 GitHub 正常现象。
...
| **DE** | 每小时 | `DE.txt` | `de.yourdomain.com` |
| **NL** | 每小时 | `NL.txt` | `nl.yourdomain.com` |
| **US_MAX** | 每小时 | `US_MAX.txt` | `usmax.yourdomain.com` |
| **一键触发所有测速** | 仅手动触发 | - (一键并行触发上述 6 个地区的测速与同步任务) | - |
| **Clean Runs**| 每 12 小时 | - (自动清理多余 Actions 日志，仅保留最新 10 条) | - |
...
1. 本项目基于公开测速算法与接口集成，所得 IP 均为 Cloudflare 官方公开的 CDN 节点 IP。请在合规及合理范围内使用。
2. **特别提示**：作者本人并无任何专业编程与开发知识，本项目中的所有代码实现、工作流修改与日常升级优化均完全依赖并由 AI 编写完成。使用过程中可能存在未知的 Bug 或异常情况，请务必谨慎评估并自行承担使用风险。
